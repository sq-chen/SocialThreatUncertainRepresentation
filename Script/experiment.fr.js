...
export async function ShowInstructions() {
    ...
    currentY = drawParagraph(globalCtx, 'Bienvenue dans cette expérience !', marginX, currentY, maxWidth, 35, tabPx);

    // 实验要求段落
    globalCtx.font = '25px Arial';
    const mainInstructionText = 'Cette expérience nécessite de déterminer les probabilités correspondant à des visages d’émotions différentes (en colère ou heureux). À chaque fois, un certain nombre de visages avec des émotions différentes seront présentés au hasard au centre et autour de l’écran, et vous devez identifier soigneusement le nombre de chaque type de visage et estimer la probabilité que chaque type de visage représente le nombre total de personnes. Après cela, une barre coulissante (de 0 % à 100 %) apparaîtra à l’écran. À ce stade, vous devez signaler la probabilité estimée et la confirmer en déplaçant la souris pour placer le curseur rouge sur la probabilité correspondante et en cliquant sur le bouton gauche de la souris.';
    currentY = drawParagraph(globalCtx, mainInstructionText, marginX, currentY, maxWidth, 35, tabPx);

    // 小节开始提醒
    const text6 = 'Avant le début de chaque sous-section, le centre de l’écran vous rappellera le type de visage (en colère/heureux) pour lequel vous devez déterminer la probabilité ensuite.';
    currentY = drawParagraph(globalCtx, text6, marginX, currentY, maxWidth, 35, tabPx);

    // 准确率说明
    const text7 = 'Veuillez estimer la probabilité aussi précisément que possible ; La précision déterminera le gain.';
    currentY = drawParagraph(globalCtx, text7, marginX, currentY, maxWidth, 35, tabPx);

    // 开始提示
    const text8 = 'Une fois que vous avez compris les exigences de l’expérience, appuyez sur la barre d’espacement pour démarrer l’experience.';
...
async function ShowBlockAnnounce(blockName) {
    ...
    if (blockName.includes('Happy')) {
        blockTypeText = 'visages heureux';
    } else if (blockName.includes('Angry')) {
        blockTypeText = 'visages en colère';
    }

    globalCtx.fillText(`Veuillez estimer avec précision le nombre de ${blockTypeText} dans ce bloc.`, centerX, currentY);
    currentY += 80; // 增加行距

    globalCtx.font = '30px Arial'; // 稍小一点的字体
    globalCtx.fillText('Appuyez sur la barre d’espacement pour commencer.', centerX, currentY);
...
export async function RestAnnounce() {
    ...
    globalCtx.fillText('Ce bloc est terminé, faites une pause, S’il vous plaît.', centerX, centerY);
...
export async function ShowEndScreen() {
    ...
    globalCtx.fillText('Expérience terminée, merci de votre participation !', centerX, centerY);
