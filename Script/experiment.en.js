 ...
 * 初始化实验环境（模拟 ptbInit）。
 * 此函数现在只负责初始化全局 p 对象。
 * @param {string|number} subjectId - 被试ID（现在可以是字母数字混合）。
 * @param {Map<string, HTMLImageElement>} loadedStimuli - 预加载的刺激图片Map。
 * @param {number} randomOrderNumber - 随机顺序数（1或2），用于决定block顺序。
 */
export function initializeExperiment(subjectId, loadedStimuli, randomOrderNumber) {
    ...
    // 直接给全局 p 对象赋值
    p.subject = subjectId;
    console.log('experiment.en.js中接收到的subjectId:', subjectId);
    console.log('p.subject赋值后:', p.subject);
    ...
    // 根据随机顺序数设置完整的实验条件顺序（替代原有的被试ID奇偶性判断）
    if (randomOrderNumber % 2 === 0) {
        // 随机数为偶数：先 AF 实验，再 CH 实验
        // AF 实验：愤怒面孔 -> 开心面孔
        // CH 实验：开心面孔 -> 愤怒面孔
        p.exp.condlabs = ['Angry_AF', 'Happy_AF', 'Happy_CH', 'Angry_CH'];
        console.log('随机顺序数', randomOrderNumber, '为偶数，设置实验顺序：AF -> CH');
    } else {
        // 随机数为奇数：先 CH 实验，再 AF 实验
        // CH 实验：愤怒面孔 -> 开心面孔
        // AF 实验：开心面孔 -> 愤怒面孔
        p.exp.condlabs = ['Angry_CH', 'Happy_CH', 'Happy_AF', 'Angry_AF'];
        console.log('随机顺序数', randomOrderNumber, '为奇数，设置实验顺序：CH -> AF');
    }
...
export async function ShowInstructions() {
    ...
    // 绘制标题（居中）
    globalCtx.fillText('Instructions', centerX, currentY);
    currentY += 70;
    globalCtx.textAlign = 'left'; // 恢复左对齐

    // 欢迎语
    globalCtx.font = '25px Arial';
    currentY = drawParagraph(globalCtx, 'Welcome to this experiment!', marginX, currentY, maxWidth, 35, tabPx);

    // 实验要求段落
    globalCtx.font = '25px Arial';
    const mainInstructionText = 'This experiment requires determining the probabilities corresponding to faces of different emotions (angry/happy). Each time a number of faces with different emotions will be randomly presented in the center and around the screen, and you need to carefully identify the number of each type of face and estimate the probability that each type of face accounts for the total number of people. After that, a sliding bar (from 0%-100%) will appear on the screen. At this point, you need to report the estimated probability and confirm it by moving the mouse to place the red cursor to the corresponding probability and clicking the left mouse button.';
    currentY = drawParagraph(globalCtx, mainInstructionText, marginX, currentY, maxWidth, 35, tabPx);

    // 小节开始提醒和准确率说明
    const text6 = 'Before the start of each subsection, the center of the screen will remind you of the type of face (angry/happy) for which you need to determine the probability next. Please estimate the probability as accurately as possible！';
    currentY = drawParagraph(globalCtx, text6, marginX, currentY, maxWidth, 35, tabPx);

    // 练习说明
    const text7 = 'There will be three practice runs to ensure you understand the experiment. If you still have questions, please contact us via email！ Thank you. (Email: yinuo.0509@gmail.com)';
    currentY = drawParagraph(globalCtx, text7, marginX, currentY, maxWidth, 35, tabPx);

    // 开始提示
    const text8 = 'Once you understand the requirements of the experiment, press the space bar to start the experiment.';
    globalCtx.font = 'bold 25px Arial';
    currentY = drawParagraph(globalCtx, text8, marginX, currentY, maxWidth, 35, tabPx);
...
// 练习前指导页面（PDF第二页）
export async function ShowPrePracticeInstructions() {
    // 清空画布并设置背景色
    globalCtx.fillStyle = 'rgb(122, 122, 122)';
    globalCtx.fillRect(0, 0, p.w.width, p.w.height);
    ...
    const maxWidth = p.w.width - marginX * 2;
    const tabPx = 48;
    let currentY = p.w.height / 2 - 100;

    // 标题
    globalCtx.textAlign = 'center';
    globalCtx.font = 'bold 45px Arial';
    const centerX = p.w.width / 2;
    globalCtx.fillText('Practice runs', centerX, currentY);
    currentY += 70;
    globalCtx.textAlign = 'left';
    ...
    globalCtx.font = '25px Arial';
    globalCtx.textAlign = 'center';
    const practiceText = 'Please estimate accurately the number of <angry faces> in this block.';
    globalCtx.fillText(practiceText, p.w.width / 2, currentY);
    currentY += 50;

    // 开始提示
    const startText = 'Press the space bar to start.';
    globalCtx.font = 'bold 25px Arial';
    globalCtx.fillText(startText, p.w.width / 2, currentY);
...
// 练习后指导页面（PDF第三页）
export async function ShowPostPracticeInstructions() {
    // 清空画布并设置背景色
    globalCtx.fillStyle = 'rgb(122, 122, 122)';
    globalCtx.fillRect(0, 0, p.w.width, p.w.height);
    ...
    globalCtx.font = 'bold 45px Arial';
    const centerX = p.w.width / 2;
    globalCtx.fillText('End of practice runs', centerX, currentY);
    currentY += 70;
    globalCtx.textAlign = 'left';
    ...
    globalCtx.font = '25px Arial';
    globalCtx.textAlign = 'center';
    const completeText1 = 'Please make sure you have already understood our experiment.';
    const completeText2 = 'Don\'t hesitate to contact us if you have any problems! (Email: yinuo.0509@gmail.com)';
    globalCtx.fillText(completeText1, p.w.width / 2, currentY);
    currentY += 35;
    globalCtx.fillText(completeText2, p.w.width / 2, currentY);
    currentY += 50;

    // 开始提示
    const startText = 'Press the space bar for the formal experiment';
    globalCtx.font = 'bold 25px Arial';
    globalCtx.fillText(startText, p.w.width / 2, currentY);
...
// 统一的练习与正式前指导序列：仅需在整个实验开始时调用一次
export async function runPracticeSequence() {
    await ShowPrePracticeInstructions();
    await RunPracticeTrials();
    await ShowPostPracticeInstructions();
    await ShowPreExperimentInstructions();
}

// 正式实验前指导页面（PDF第四页）
export async function ShowPreExperimentInstructions() {
    // 清空画布并设置背景色
    globalCtx.fillStyle = 'rgb(122, 122, 122)';
    globalCtx.fillRect(0, 0, p.w.width, p.w.height);
    ...
    globalCtx.font = 'bold 45px Arial';
    const centerX = p.w.width / 2;
    globalCtx.fillText('Formal runs', centerX, currentY);
    currentY += 70;
    globalCtx.textAlign = 'left';
...
export async function RunPracticeTrials() {
    ...
    // 运行3个练习试次
    for (let practiceTrial = 0; practiceTrial < 3; practiceTrial++) {
        console.log(`练习试次 ${practiceTrial + 1}/3`);
        
        // 1. 练习部分跳过注视点，直接开始面孔呈现
        
        // 2. 呈现面部池 (Faces Pool) - 与RunTrial结构一致
        // 清空画布并设置背景色
        globalCtx.fillStyle = 'rgb(122, 122, 122)';
        globalCtx.fillRect(0, 0, p.w.width, p.w.height);

        // 随机生成面孔数量和比例
        const faceNum = Math.floor(Math.random() * 8) + 8; // 8-15个面孔
        const proportion = Math.random(); // 0-1之间的随机比例
        const target_num = Math.round(faceNum * proportion);
        const nontarget_num = faceNum - target_num;
        const objective_p = target_num / faceNum;

        // 练习部分固定选择愤怒面孔的概率
        const targetEmotion = 'A'; // 愤怒面孔
        const nontargetEmotion = 'H'; // 快乐面孔
        ...
        ethnicities.forEach(ethnicity => {
            genders.forEach(gender => {
                for (let i = 1; i <= 8; i++) {
                    ...
                    const happyPath = `stimuli/${ethnicity}_H_${gender}_${numStr}.png`;
                    const angryPath = `stimuli/${ethnicity}_A_${gender}_${numStr}.png`;

                    const happyImg = globalLoadedStimuli.get(happyPath);
                    const angryImg = globalLoadedStimuli.get(angryPath);
        ...
        // 4. 评分 (VAS) - 与RunTrial完全一致，但使用随机初始值
        const randomVasInit = Math.floor(Math.random() * 101); // 0-100之间的随机整数
        const vasResult = await getVASRating(globalCtx, p.vas, randomVasInit); // 初始值设为随机值
        ...
        console.log(`练习试次 ${practiceTrial + 1}: 估计概率 ${vasRating}%, 实际概率 ${Math.round(proportion * 100)}%`);

        // 5. ITI (试次间间隔) - 与RunTrial完全一致
        // 清空画布并设置背景色
        globalCtx.fillStyle = 'rgb(122, 122, 122)';
        globalCtx.fillRect(0, 0, p.w.width, p.w.height);
...
async function ShowBlockAnnounce(blockName) {
    ...
    if (blockName.includes('Happy')) {
        blockTypeText = 'happy faces';
    } else if (blockName.includes('Angry')) {
        blockTypeText = 'angry faces';
    }

    globalCtx.fillText(`Please accurately estimate the number of <${blockTypeText}> in this block`, centerX, currentY);
    currentY += 80; // 增加行距

    globalCtx.font = '30px Arial'; // 稍小一点的字体
    globalCtx.fillText('Press the spacebar to start the experiment', centerX, currentY);
...
export async function RestAnnounce() {
    ...
    globalCtx.fillText('This block is over, please take a break', centerX, centerY);
...
export async function ShowEndScreen() {
    ...
    // 三行英文提示（与前面指导语相同的画布风格，居中一行一句）
    globalCtx.fillText('You have successfully finished the task,', centerX, centerY - 40);
    globalCtx.fillText('Thank you for your time!', centerX, centerY);
    globalCtx.fillText('Please return to Qualtrics to finish following questionnaires', centerX, centerY + 40);
...
export async function savePStructureAsJson(subjectId) {
    try {
        ...
        if (!p || Object.keys(p).length === 0) {
            ...
            alert('Experiment data is empty, cannot save JSON file');
            return false;
        }
        ...
        // 检查是否为纯数字ID，如果是则保持原有格式化，否则直接使用字符串
        const isNumericId = /^\d+$/.test(String(subjectId));
        const filename = isNumericId 
            ? `sub${String(subjectId).padStart(2, '0')}_p_data.json`
            : `sub${String(subjectId)}_p_data.json`;
        ...
        if (response.ok) {
            ...
            if (result.success) {
                ...
                //alert(`Experiment data successfully saved to server!\nFile path: ${result.filePath}`);
                return true;
            } else {
...
export async function runExperimentCH() {
    ...
    for (let b = 0; b < chBlockIndices.length; b++) {
        ...
        // 在每个块结束后保存 CSV 数据
        console.log('生成文件名时的p.subject:', p.subject);
        console.log('p.subject类型:', typeof p.subject);
        const isNumericId = /^\d+$/.test(String(p.subject));
        console.log('是否为数字ID:', isNumericId);
        const filename = isNumericId 
            ? `sub${String(p.subject).padStart(2, '0')}_${blockName}.csv`
            : `sub${String(p.subject)}_${blockName}.csv`;
